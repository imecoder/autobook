...
def get_cookies_from_chrome(domain):
    sql = f'SELECT name, encrypted_value, host_key FROM cookies where host_key like "%{domain}%"'
    print(sql)
    ...
    cookie = {}
    for row in cur:
        print(row['name'] + '=' + chrome_decrypt(row['encrypted_value']) + ', ' + row['host_key'])
        # print(name)
        # name = row['name']
        # cookie[name]['encrypted_value']=chrome_decrypt(row['encrypted_value'])
        # cookie[name]['host_key']=chrome_decrypt(row['host_key'])
