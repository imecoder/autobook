...
def test_brush(sessionid):
    ret, msg = execute_instruction(sessionid, 'A26NOVFRATAO/LH')
    if ret == False:
        myflag.set_flag_relogin(True)
        if 'text' not in msg:
            logger.warning('刷票失败')
...
def test_occupy(sessionid) :
    ret, msg = execute_instruction(sessionid, '01Y1')
    if ret == False:
        logger.warning('占票失败')
        return False
...
if __name__ == '__main__':
    ...
    while True :
        t1 = Thread(target=test_brush, args=(sessionid,))
        t2 = Thread(target=test_occupy, args=(sessionid,))
